...
The current template-creation meta-template is `RA9cOdAwjxTF6qNNFkXTl_EgOan0LDPmu0fWnIgmZu-7E` ("Defining an assertion template (new version)"); prefer replicating its statement shapes when hand-authoring a template. Its own identity node is `sub:template` while the placeholder for the template being created is `sub:newtemplate` — keep those two roles on distinct IRIs when a template's form creates another template, otherwise labels and descriptions of the two roles merge ambiguously on one node.
...
The current view-creation template is `RAfa1tHtbOrV0YaCY0LyjHkpXoudrauhN8DUYPM63ZQ-k` ("Declaring a resource view", in [assertion-templates/](assertion-templates/)); prefer replicating its statement shapes when hand-authoring a view. It is the **single** template for resource views since 2026-09-08: it carries `gen:governedBy` as an optional statement (so it covers space-governed views, see below), the full view-type list, and the three optional fill-query statements (see "Fill queries" under view actions). Its action group's target field, part field, query mapping **and `gen:isVisibleTo`** are optional statements — omit a triple when it does not apply; do not write the literal `"void"` (older views carry it, Nanodash still reads it as "absent", and a supersede through the form round-trips it verbatim unless the row is cleared). **History, for reading older views' `nt:wasCreatedFromTemplate`:** two chains existed until 2026-09-08 — the plain one (`RARLsTlq…` → `RA8_hijw…` → `RA7j_m2ml…` → `RAJ2Ssnt…` → `RA4QvSuNS…`, ~200 live views) and the governed-by variant (`RAr1Krh9…` → `RA7FuFF5…` → `RAK4W3mf…`, ~27). They are signed with different keys, so they could not be merged by `npx:supersedes`; the governed-by one became the generic template (it consumes every view the plain one does), and the plain head is now `RAXFrtnNYw2ns09arwNhIpA87WP3aNO2AIIckhjydKJGI` ("Declaring a resource view (deprecated)", `nt:UnlistedTemplate`): `template-version=latest` on the ~200 views made from that chain still resolves to it, so they remain updatable, but it is no longer offered for new views (`TemplateData.refreshTemplates` skips unlisted templates). `RAZqKZpa…` ("experimental") is unlisted likewise. Two heads still appear in the list and predate both chains: `RARLsTlq…` (Feb 2026, ~124 live views) and `RAtgc4BB…` ("with alternative viewType choices"); they are signed with Ashley Caselli's key, so only she can unlist them. The generic head itself is a fork collapse: two agents superseded `RAK4W3mf…` independently twenty minutes apart, and `RAfa1tHt…` supersedes both heads. **Before superseding any shared template, resolve the head with `get-latest-version-of-np` and check nobody else is mid-edit on the same chain.** Do not add `nt:hasDatatype xsd:integer` to `pageSize`: `ValueFiller` matches literal datatypes strictly, so the typed placeholder would reject the plain `"10"` every version has produced.
...
- **Fill queries — pre-filling from the target resource** (`gen:hasActionFillQuery`, nanodash issue #690, PR #693, 2026-09-08). The listing mapping above reads the view's *rows*, which is the wrong source for a **default inherited from the resource the page is about** — with no rows it fills nothing, and with *n* rows it fills the same value *n* times. A result action can instead carry its **own query, bound to the target**, whose **first row** pre-fills the form when it opens:
  ...
  - **Template support:** the view-creation template offers the three statements as optional rows of the action group (since 2026-09-08; current head `RAfa1tHt…`); the fill query is a guided choice over published queries. Requires a Nanodash with PR #693 (released 2026-09-08); older instances ignore the predicates (button without pre-fill).
...
| **Revoke a role from an agent** | embedded node: `<rev> a gen:RevokedRoleInstantiation ; gen:forSpace <space> ; gen:forAgent <agent> ; gen:hasRole <role>` (use `gen:AdminRole` to revoke adminship — the admin tier has no per-role IRI, so one shape covers every tier). Nanopub type `gen:RevokedRoleInstantiation`. **See the revocation rules below.** | `RAgP7HztxeW0FSEctIy2fla6YTf8HTbyXRgYpXWc3McMU` |
| **Detach a role from a Space** | `<space> gen:detachedRole <role>` — the stative antonym of `gen:hasRole`, keyed on `(space, role)`. Admin-authored and latest-wins against **direct *and* preset-derived** attachments, so it removes the role's availability and cascades to every instantiation anchored on it. | `RAVnwxKEiHRthdz-ZUNDGoDz9CDiTjnx4gEzs4lCXg6gk` |
