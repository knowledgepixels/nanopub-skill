...
The current view-creation template is `RAJ2SsntOHN7leMyEXoyBoUp6l2sd-VAPCpPrHVQa2QVM` ("Declaring a resource view", in [assertion-templates/](assertion-templates/)); prefer replicating its statement shapes when hand-authoring a view. Since this version (2026-09-07, nanodash issue #691) the action group's target field, part field and query mapping are **optional statements**: simply omit the triple when it does not apply. Earlier versions could not express that and used the literal `"void"` instead, which Nanodash still reads as "absent" — published views carry it forever, and a supersede through the form round-trips it verbatim unless the row is cleared. For a view declaring `gen:governedBy` (space-governed versions, see below), use the derived variant `RA7FuFF5RWV_iVg2IQNcFITP-ReH8Py1QQQcvD3mtku5g` ("Declaring a resource view (with governed-by)") — **note** that this variant, like the header-view template, has *not* had the cleanup yet and still wants `"void"` in the unused action fields. For a query-less **header view**, use `RAZYU_kQZ0B3ruoNSeZOaKR6M93PJ-z1jlArXl7_H4EEU` ("Declaring a header view") — see the dedicated "Header views" section below. For a view whose query computes an inline diagram, see the "SVG views" section below.
...
- **`gen:isVisibleTo`** restricts the button to viewers holding a role **tier** (`gen:AdminRole` / `gen:MaintainerRole` / `gen:MemberRole` / `gen:ObserverRole` — matches that tier *or above* in the resource's governing space) or a **specific role IRI** (exact holders only). Multiple values are OR. Omission or `gen:EveryoneRole` means visible to everyone — `gen:EveryoneRole` is a Nanodash-side sentinel (never a grantable tier), kept as the template's default for this statement. On a user page the owner counts as sole admin. This is relevance-gating, not a security boundary — publishing authority is still checked server-side.
...
- **Fill queries — pre-filling from the target resource** (`gen:hasActionFillQuery`, nanodash issue #690, PR #693, 2026-09-08). The listing mapping above reads the view's *rows*, which is the wrong source for a **default inherited from the resource the page is about** — with no rows it fills nothing, and with *n* rows it fills the same value *n* times. A result action can instead carry its **own query, bound to the target**, whose **first row** pre-fills the form when it opens:
  ...
  - **Template support:** the view-creation template does not yet offer these three statements, so a view using them must be **hand-authored** (the checker will report the three triples as unmatched — expected). Requires a Nanodash with PR #693; older instances ignore the predicates (button without pre-fill).
