...
- **The pin also beats `npx:supersedes`.** When the version a consumer references carries `gen:governedBy` but governed resolution returns nothing, Nanodash keeps that exact version and does not fall back to the supersedes chain, for views (the `list-view-displays` resolver) and templates (`TemplateData.getLatestTemplateId`) alike. So publish the kind registration before, or together with, the first governed version, and never re-mint the kind on a later version.
