...
The current template-creation meta-template is `RA8_1MbQzRcWYAFAX-3eLloCK9bUksswnu5bs90yl_Ozw` ("Defining an assertion template (new version)"); prefer replicating its statement shapes when hand-authoring a template. Its own identity node is `sub:template` while the placeholder for the template being created is `sub:newtemplate` — keep those two roles on distinct IRIs when a template's form creates another template, otherwise labels and descriptions of the two roles merge ambiguously on one node.
