...
- **Embedded identity** (preferred for new templates): the template node is a regular embedded IRI with any local name (`sub:my-template a nt:AssertionTemplate ; …`), declared **`npx:embeds sub:my-template`** in pubinfo — the embeds triple is required (type inference, listings, and governed resolution all key on it). External references (`nt:wasCreatedFromTemplate`, `template=` publish URLs) then carry the embedded IRI, not the nanopub URI. Optionally the node declares a stable **kind** (`dct:isVersionOf <kind>`, with `npx:introduces <kind>` in pubinfo — same rules as view kinds: use the original full URI on later versions) and **`gen:governedBy <space>`** (see space-governed definition versions below). Same pattern applies to `nt:ProvenanceTemplate` / `nt:PubinfoTemplate`. Re-deriving a later version from a published one, keep that full kind URI rather than turning it back into `sub:templateKind`: a later version with `sub:templateKind` mints a fresh, unregistered kind, and for a governed template every nanopub created from it stays pinned to it (this happened to the view template `RApNBmFs…` on 2026-09-09).
...
python3 scripts/governance_check.py <file.trig> [...]            # governed versions: is the kind registered?
```
...
Both conformance checkers also run `governance_check.py` on the file. For every **space-governed definition** in it (an embedded node declaring `dct:isVersionOf` and `gen:governedBy`), that script looks up whether the kind is registered as a maintained resource of the named space. It reads the live spaces repos of the public query instances, trying each in turn; set `SPACES_REPOS` to override them, or `SKIP_GOVERNANCE_CHECK=1` to skip the lookup. A registered kind prints a `governed:` line. The following print a `NOTE governed:` line:
- a kind that is not registered with that space (the note names any other space it is registered with);
- a kind minted by the unpublished file itself, which can only be registered after publishing;
...
The notes never fail a check: a governed version legitimately waits for an admin's registration, for instance when a member or a bot published it.
...
`pin` is the nanopub URI of the version a reference points at. The result has at most one row (`version`, `date`, `np`, `mode`); `mode` says which rule applied: `governed` (the kind is registered with the space) or `supersedes` (it is not; see below). The query is type-agnostic (views, templates, and any future definition kind following the embeds/isVersionOf/governedBy convention). Nanodash up to 5.15.0 uses its predecessor `RAPSWgzHef9bIJyCoLodFH-BWtlESf1jIstEb0kn4B5Cw`, which takes only kind and space. Notes:

- **No `npx:supersedes` needed**: governed versions float purely by publication time and space membership, so a *different* member can publish the next canonical version — the escape hatch for definitions whose original signing key is unavailable. If the kind is registered but no current member has a version, the result is empty and the caller keeps its pinned version (the pin is the floor).
- **`gen:governedBy` has no effect until the kind is registered** as maintained by that space, and versions signed by non-members are skipped. Until then the pinned version resolves like an ungoverned one, along its own same-key supersedes chain, by the same rule as `get-latest-version-of-np` (a chain that has forked into several heads resolves to nothing). The registration is what lets the space's members publish the next versions; it also lists the kind in the space's maintained templates and views and gives it a maintained-resource page. The conformance checkers report whether a governed version's kind is registered (see *Checking a nanopub against its template* above).
- **Up to Nanodash 5.15.0 an unregistered kind freezes the pin.** There, the predecessor query returns nothing for an unregistered kind, and Nanodash then keeps the exact version referenced, ignoring even same-key `npx:supersedes`, for views and templates alike. nanodash#732 fixed this (merged 2026-09-24, in the first release after 5.15.0). The server-side view-display listing queries still show such a pin as is; rendering does not depend on them. So publish the kind registration before, or together with, the first governed version.
- **Never re-mint the kind on a later version.** A new version keeps the kind IRI (and `npx:introduces` it again). A version with a new kind starts a separate line that governed resolution of the earlier versions does not see. Registering that accidental kind would then pin every reference to it, so republish with the original kind instead. The conformance checkers flag a governed version whose kind differs from the version it supersedes.
